...
function [xy_data] = Extr_Data_RectangleeRounded_Holes(L, ri, ro, varargin)
...
xy_data3 = [cosd(-90:1:90); sind(-90:1:90)]'*ro+[L 0];
xy_data4 = [cosd(90); sind(90)]'*ro;
...
xy_data8 = [cosd(90:-1:-270); sind(90:-1:-270)]'*ri;
xy_data9 = [cosd(-270:1:-90); sind(-270:1:-90)]'*ro;

%xy_data = [xy_data1; xy_data2; xy_data3; xy_data4; xy_data5;xy_data6; xy_data7;xy_data8;xy_data9];
xy_data = [xy_data1; xy_data2; xy_data3; xy_data4 ; xy_data8; xy_data9]
...
if (nargin == 0 || strcmpi(showplot,'plot'))
   ...
   % text(xL*L*1.1,H/2,'{\color{blue}H}');
    ...
    text(ri*cosd(-180)*0.5,0,'{\color{green}ro}');
   % plot([0 xL*L],[-1 -1]*ri/2,'m-d','MarkerFaceColor','m');
   % text(xL*L/2,-ri/2-H/15,'{\color{magenta}xL*L}');
    
    title('[xy\_data] = Extr\_Data\_RectangleRounded\_Holes(L, ri, ro);');
    hold off
    box on
    clear xy_data
end
